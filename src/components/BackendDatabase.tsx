...
export function BackendDatabase() {
  ...
  return (
    <section id="backend-database" className="relative py-16 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          >
            <div className="bg-slate-900/80 backdrop-blur-xl border border-red-500/20 rounded-lg overflow-hidden">
              <div className="bg-slate-800/50 border-b border-red-500/20 px-4 py-3">
                <div className="flex items-center gap-2">
                  <Table className="w-4 h-4 text-red-400" />
                  <span className="text-red-400 text-sm font-mono">Tables</span>
                </div>
              </div>
              ...
              <div className="border-t border-red-500/20 p-4">
                <div className="text-xs text-slate-400 font-mono">
                  <div className="mb-2 text-red-400">Schema:</div>
                  {activeTable && (
            ...
            {/* Quick Queries */}
            <div className="mt-4 bg-slate-900/80 backdrop-blur-xl border border-red-500/20 rounded-lg overflow-hidden">
              <div className="bg-slate-800/50 border-b border-red-500/20 px-4 py-3">
                <div className="flex items-center gap-2">
                  <FileCode className="w-4 h-4 text-red-400" />
                  <span className="text-red-400 text-sm font-mono">Quick Queries</span>
                </div>
              </div>
              <div className="p-2 space-y-1">
                {sampleQueries.map((query, index) => (
                  <button
                    ...
                    className="w-full text-left px-3 py-2 rounded-lg font-mono text-xs text-slate-400 hover:bg-slate-800/50 hover:text-slate-300 transition-colors"
                  >
                    {query}
                  </button>
                ))}
          ...
          >
            {/* Query Editor */}
            <div className="bg-slate-900/80 backdrop-blur-xl border border-red-500/20 rounded-lg overflow-hidden">
              <div className="bg-slate-800/50 border-b border-red-500/20 px-4 py-3 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Search className="w-4 h-4 text-red-400" />
                  <span className="text-red-400 text-sm font-mono">SQL Query Editor</span>
                </div>
                <button
                  onClick={handleExecute}
                  disabled={isExecuting}
                  className="flex items-center gap-2 px-4 py-1.5 bg-red-500/20 hover:bg-red-500/30 text-red-400 rounded-lg border border-red-500/30 font-mono text-sm transition-colors disabled:opacity-50"
                >
              ...
              <div className="p-4">
                <textarea
                  ...
                  className="w-full h-32 bg-slate-950/50 text-green-400 font-mono text-sm p-4 rounded-lg border border-slate-700 focus:border-red-500/50 outline-none resize-none"
            ...
            <AnimatePresence mode="wait">
              {isExecuting ? (
                <motion.div
                  ...
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  className="bg-slate-900/80 backdrop-blur-xl border border-red-500/20 rounded-lg p-8 text-center"
                >
                  <div className="inline-block animate-spin rounded-full h-8 w-8 border-4 border-red-400 border-t-transparent mb-4" />
                ...
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0 }}
                  className="bg-slate-900/80 backdrop-blur-xl border border-red-500/20 rounded-lg overflow-hidden"
                >
                  {/* Results Header */}
                  <div className="bg-slate-800/50 border-b border-red-500/20 px-4 py-3 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-green-400 text-sm font-mono">Query Successful</span>
                      </div>
                        ...
                        <thead>
                          <tr className="border-b border-slate-700">
                            {queryResult.columns.map(col => (
                              <th
                                key={col}
                                className="px-4 py-3 text-left text-red-400 bg-slate-800/30 whitespace-nowrap"
                              >
                                {col}
                              </th>
                            ))}
                ...
                <motion.div
                  ...
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  className="bg-slate-900/80 backdrop-blur-xl border border-red-500/20 rounded-lg p-12 text-center"
                >
                  <Database className="w-16 h-16 text-slate-600 mx-auto mb-4" />
                  <div className="text-slate-400 font-mono text-sm mb-2">No query executed yet</div>
